...
class SubjectRow(ButtonBehavior, BoxLayout):
    ...
    def __init__(self, subject: dict, on_press_row, on_edit=None, **kwargs):
        ...
        # Заголовок + кнопка «✎» (редактировать карточку) в одну строку.
        # Кнопка «✎» — отдельный Button: тап по ней НЕ открывает занятие
        # (дочерний Button поглощает касание, on_release строки не срабатывает).
        title_row = BoxLayout(orientation="horizontal", size_hint_y=None, height=34,
                              spacing=6)
        title = Label(
        ...
        edit_btn = Button(text="✎", size_hint=(None, None), size=(40, 30),
                          background_color=(0.85, 0.85, 0.9, 1))
        edit_btn.bind(on_release=lambda *_: (self._on_edit and self._on_edit(self.subject)))
...
class HomeScreen(Screen):
    ...
    def on_pre_enter(self, *_):
        ...
        add_btn.bind(on_release=self._add_subject)
        ...
        if not subjects:
            ...
            for subj in subjects:
                container.add_widget(SubjectRow(
                    subj, on_press_row=self._open_lesson, on_edit=self._edit_subject))
    ...
    def _add_subject(self, *_):
        """«+ Предмет» → карточка в режиме СОЗДАНИЯ (сбрасываем режим редактирования)."""
        add_screen = self.manager.get_screen("add_subject")
        add_screen.edit_subject_id = None
        self.manager.current = "add_subject"

    def _edit_subject(self, subject: dict):
        """«✎» на строке предмета → карточка в режиме РЕДАКТИРОВАНИЯ этого предмета."""
        add_screen = self.manager.get_screen("add_subject")
        add_screen.edit_subject_id = subject["id"]
        self.manager.current = "add_subject"
...
class AddSubjectScreen(Screen):
    """
    Карточка предмета: название, план в занятиях, выбор/создание группы.
    Работает в ДВУХ режимах:
      * создание нового предмета (edit_subject_id is None) — по «+ Предмет»;
      * редактирование существующего (edit_subject_id задан) — по «✎» в списке.
    Режим задаётся установкой self.edit_subject_id ПЕРЕД переходом на экран.
    """

    edit_subject_id = None  # None → создание; int → редактируем этот предмет

    def on_pre_enter(self, *_):
        self.clear_widgets()
        editing = self.edit_subject_id is not None

        # В режиме редактирования подтягиваем текущую карточку.
        card = None
        if editing:
            db = get_db()
            card = db.get_subject(self.edit_subject_id)
            db.close()
            if card is None:
                # Предмет исчез (напр., удалён) — выходим в режим создания.
                self.edit_subject_id = None
                editing = False

        root = BoxLayout(orientation="vertical", padding=16, spacing=10)
        root.add_widget(Label(
            text="[b]Редактирование предмета[/b]" if editing else "[b]Новый предмет[/b]",
        ...
        self.name_input = TextInput(
            ...
            text=(card["name"] if editing else ""),
        )
        ...
        root.add_widget(Label(text="План (в занятиях):", size_hint_y=None, height=22,
                              halign="left", color=(0.2, 0.2, 0.2, 1)))
        self.planned_input = TextInput(
            multiline=False, input_filter="int", size_hint_y=None, height=40,
            text=(str(card["planned"]) if editing else "0"),
        )
        ...
        db.close()
        # В режиме редактирования по умолчанию выбрана текущая группа предмета.
        if editing and card["group_name"] in groups:
            default_group = card["group_name"]
        elif groups:
            default_group = groups[0]
        else:
            default_group = NEW_GROUP_OPTION
        ...
        # Подсказка при редактировании: смена названия/группы рассинхронит schedule.txt.
        if editing:
            root.add_widget(Label(
                text="[i]Меняя название или группу, обновите их и в schedule.txt —\n"
                     "там предмет ищется по названию и группе.[/i]",
                markup=True, size_hint_y=None, height=40, font_size=12,
                color=(0.5, 0.5, 0.5, 1), halign="center", valign="middle",
            ))
    ...
    def _save(self, *_):
        ...
        if self.edit_subject_id is not None:
            db = get_db()
            try:
                db.update_subject(self.edit_subject_id, name, planned, group)
            except ValueError as exc:
                db.close()
                self.status.text = str(exc)
                return
            db.close()
            self.edit_subject_id = None  # сбрасываем режим
...
class TodayScheduleScreen(Screen):
    ...
    def on_pre_enter(self, *_):
        self.clear_widgets()
        # Дата, за которую показываем расписание (по умолчанию — сегодня).
        # Занятия из schedule.txt генерятся от даты семестра, поэтому нужна
        # навигация по датам, иначе загруженное расписание «не видно» на сегодня.
        if getattr(self, "view_date", None) is None:
        ...
        header.add_widget(Label(text="[b]Расписание[/b]", markup=True, font_size=20,
                                color=(0.1, 0.1, 0.1, 1)))
        root.add_widget(header)

        # Панель навигации по датам: ‹ пред. день · дата (+«Сегодня») · след. день ›
        nav = BoxLayout(orientation="horizontal", size_hint_y=None, height=40,
                        padding=(10, 2), spacing=8)
        prev_btn = Button(text="‹", size_hint_x=None, width=48)
        prev_btn.bind(on_release=lambda *_: self._shift_date(-1))
        next_btn = Button(text="›", size_hint_x=None, width=48)
        next_btn.bind(on_release=lambda *_: self._shift_date(+1))
        is_today = self.view_date == date.today()
        date_caption = self.view_date.strftime("%a %d.%m.%Y")
        if is_today:
            date_caption += "  (сегодня)"
        date_lbl = Label(text=date_caption, color=(0.25, 0.25, 0.25, 1))
        nav.add_widget(prev_btn)
        nav.add_widget(date_lbl)
        if not is_today:
            today_btn = Button(text="Сегодня", size_hint_x=None, width=90)
            today_btn.bind(on_release=lambda *_: self._go_to_date(date.today()))
            nav.add_widget(today_btn)
        nav.add_widget(next_btn)
        root.add_widget(nav)

        db = get_db()
        lessons = db.list_schedule_for_date(self.view_date.isoformat())
        db.close()
        ...
        box.bind(minimum_height=box.setter("height"))
        if not lessons:
            empty_txt = ("На этот день занятий нет.\n\nПереключите дату стрелками ‹ ›, "
                         "нажмите «📅 Загрузить расписание»\nили «➕ ДОБАВИТЬ ЗАНЯТИЕ».")
        ...
        # Кнопки навигации по датам продублированы в панели nav выше.
        load_sched = Button(text="📅 Загрузить расписание", size_hint_x=None, width=210,
                            background_color=(0.55, 0.45, 0.8, 1))
        load_sched.bind(on_release=self._load_schedule)
        export = Button(text="📤 В календарь (.ics)", size_hint_x=None, width=190,
                        background_color=(0.35, 0.55, 0.85, 1))
        export.bind(on_release=self._export_calendar)
        actions.add_widget(add_les)
        actions.add_widget(load_sched)
    ...
    def _shift_date(self, days: int):
        """Сдвинуть просматриваемую дату на ±1 день и перерисовать экран."""
        self.view_date = (getattr(self, "view_date", None) or date.today()) + timedelta(days=days)
        self.on_pre_enter()

    def _go_to_date(self, target: date):
        """Перейти к конкретной дате и перерисовать экран."""
    ...
    def _add_lesson_dialog(self, *_):
        """➕ Внеочередное занятие на просматриваемую дату: выбор предмета + время начала."""
        db = get_db()
        subjects = db.list_subjects()
        db.close()
        names = [f"{s['name']} ({s['group_name']})" for s in subjects]
        ...
        def _do(*_):
            if spinner.text in id_by_label:
                ...
                # Занятие добавляем на ПРОСМАТРИВАЕМУЮ дату (не всегда сегодня).
                view_day = getattr(self, "view_date", None) or date.today()
                held_at = f"{view_day.isoformat()}T{hhmm}:00"
                db2 = get_db()
    ...
    def _load_schedule(self, *_):
        """
        📅 Загрузить расписание из файла schedule.txt: разобрать, сгенерировать
        занятия (план+1, с учётом Ч/Н) и заменить прежние 'scheduled'-занятия в БД.
        Проведённые/отменённые занятия не трогаются.
        """
        import os

        from schedule_file import (ScheduleError, generate_occurrences,
                                    parse_schedule)

        path = "schedule.txt"
        if not os.path.exists(path):
            Popup(title="Расписание",
                  content=Label(text=f"Файл '{path}' не найден.\n\nСоздайте его рядом с "
                                     f"приложением и заполните\n(см. docs/SCHEDULE.md)."),
        ...
        try:
            ...
            db = get_db()
            plans = db.subject_plans_map()
            occ = generate_occurrences(parsed, plans)
            result = db.replace_scheduled_lessons(occ)
            db.close()
        except ScheduleError as exc:
            Popup(title="Ошибка в расписании",
                  content=Label(text=f"Не удалось разобрать '{path}':\n\n{exc}"),
                  size_hint=(0.9, 0.5)).open()
            return
        except Exception as exc:  # noqa: BLE001 — показать пользователю любую иную ошибку
            Popup(title="Ошибка загрузки расписания",
                  content=Label(text=str(exc)), size_hint=(0.9, 0.5)).open()
            return

        # Занятия из расписания генерятся от даты семестра (может быть в будущем).
        # Чтобы загруженное было сразу видно, перепрыгиваем на дату первого занятия.
        if occ:
            first_date = occ[0]["held_at"][:10]
            try:
                self.view_date = date.fromisoformat(first_date)
            except ValueError:
                pass
        msg = (f"Загружено занятий: {result['added']}\n"
               f"Удалено прежних (запланированных): {result['removed']}")
        if occ:
            msg += (f"\n\nПервое занятие — {self.view_date.strftime('%d.%m.%Y')} "
                    f"(экран переключён на эту дату).\n"
                    f"Листайте даты стрелками ‹ ›.")
        if result["skipped"]:
            msg += ("\n\nПропущены (нет такого предмета/группы в карточках):\n  "
                    + "\n  ".join(result["skipped"]))
        Popup(title="Расписание загружено",
              content=Label(text=msg), size_hint=(0.9, 0.5)).open()
        self.on_pre_enter()
...
class GradebookScreen(Screen):
    ...
    def on_pre_enter(self, *_):
        ...
        header.add_widget(Label(text="[b]Ведомость[/b]", markup=True, font_size=20,
                                color=(0.1, 0.1, 0.1, 1)))
        xlsx_btn = Button(text="Excel", size_hint_x=None, width=70,
                          background_color=(0.2, 0.6, 0.3, 1))
        xlsx_btn.bind(on_release=lambda *_: self._export_report("xlsx"))
        pdf_btn = Button(text="PDF", size_hint_x=None, width=60,
                         background_color=(0.75, 0.3, 0.3, 1))
        pdf_btn.bind(on_release=lambda *_: self._export_report("pdf"))
        header.add_widget(xlsx_btn)
    ...
    def _export_report(self, fmt: str):
        """
        Выгрузка ведомости текущего предмета в Excel (.xlsx) или PDF.
        Тяжёлые библиотеки (openpyxl/reportlab) импортируются внутри report_export
        лениво; при их отсутствии показываем понятное сообщение, не роняя приложение.
        """
        ...
        # Безопасное имя файла из названия предмета.
        safe = "".join(c if c.isalnum() or c in " -_" else "_"
                       for c in (summary.get("name", "report") if summary else "report")).strip()
        try:
            import report_export
            if fmt == "xlsx":
                data = report_export.build_xlsx(summary, gradebook)
                path = f"vedomost_{safe}.xlsx"
            else:
                data = report_export.build_pdf(summary, gradebook)
                path = f"vedomost_{safe}.pdf"
            with open(path, "wb") as f:
                f.write(data)
            msg = f"Ведомость сохранена в файл:\n{path}"
        except ImportError as e:
            msg = (f"Не установлена библиотека для {fmt.upper()}:\n{e}\n\n"
                   f"Установите: pip install "
                   f"{'openpyxl' if fmt == 'xlsx' else 'reportlab'}")
        except Exception as e:  # noqa: BLE001
            msg = f"Ошибка формирования {fmt.upper()}:\n{type(e).__name__}: {e}"
        Popup(title="Экспорт ведомости",
              content=Label(text=msg), size_hint=(0.9, 0.4)).open()
