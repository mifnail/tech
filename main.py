...
class LessonScreen(Screen):
    ...
    def _show_attendance(self, *_):
        ...
        self._body.add_widget(Label(
            ...
            font_size=12, color=(0.4, 0.4, 0.4, 1), size_hint_y=None, height=24,
        ))
        ...
        for st in students:
            row = StudentAttendanceRow(st)
            self._rows.append(row)
            box.add_widget(row)
        scroll.add_widget(box)
        self._body.add_widget(scroll)

        # Кнопка сохранения занятия и всех отметок.
        save = Button(text="Сохранить занятие", size_hint_y=None, height=48,
                      background_color=(0.4, 0.7, 0.4, 1))
        save.bind(on_release=self._save_lesson)
        self._body.add_widget(save)

    def _save_lesson(self, *_):
        """Создать занятие и сохранить отметки. Неотмеченные = отсутствующие."""
        db = get_db()
        lesson_id = db.add_lesson(self.subject["id"], datetime.now().isoformat())
        for row in self._rows:
            st = row.att_state
            if st == "absent":
                db.mark_attendance(lesson_id, row.student["id"], present=False, grade=None)
            elif st == "present":
                db.mark_attendance(lesson_id, row.student["id"], present=True, grade=None)
            else:  # оценка
                db.mark_attendance(lesson_id, row.student["id"], present=True, grade=int(st))
        db.close()

        # Короткое подтверждение и возврат на главный (прогресс-бар обновится).
        Popup(title="Готово",
